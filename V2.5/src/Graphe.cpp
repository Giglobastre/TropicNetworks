...
void Graphe::Charger_images(std::string nomfile)
{
    if(nomfile=="Graphe_1")
    {
        BITMAP *vache;
        BITMAP *homme;
        BITMAP *herbe;
        vache=load_bitmap("Images/vache.bmp",NULL);
        homme=load_bitmap("Images/homme.bmp",NULL);
        herbe=load_bitmap("Images/herbe.bmp",NULL);
    ...
    while(var_quit_affiche!=1)
    {
        clear_bitmap(page);
        {
            for(int i=0 ; i<m_ordre ; i++)
            {
                ...
                //circlefill(page,m_Sommets[i].getPos_x(),m_Sommets[i].getPos_y(),20,makecol(255,255,255));
                //affichage des images
                if(nomfile=="graphe_1.txt")
                {
                    BITMAP *vache;
                    BITMAP *homme;
                    BITMAP *herbe;
                    ...
                    homme=load_bitmap("Images/homme.bmp",NULL);
                    herbe=load_bitmap("Images/herbe.bmp",NULL);
                    //faire un if pour tester si sommet actif
                    if(i==0 && var==0 && m_Sommets[i].getSante()!=0)
                    {
                        m_Sommets[i].setActif(true);
                        draw_sprite(page,vache,m_Sommets[i].getPos_x()-100,m_Sommets[i].getPos_y()-100);
                    }
                    if(i==1 && var>=1 && m_Sommets[i].getSante()!=0)
                    {
                        m_Sommets[i].setActif(true);
                        draw_sprite(page,homme,m_Sommets[i].getPos_x()-100,m_Sommets[i].getPos_y()-100);
                    }
                    if(i==2 && var>=2 && m_Sommets[i].getSante()!=0)
                    {
                        m_Sommets[i].setActif(true);
                        draw_sprite(page,herbe,m_Sommets[i].getPos_x()-100,m_Sommets[i].getPos_y()-100);
                    }
                }

                if(nomfile=="graphe_2.txt")
                {
                    for(int i=0; i<m_ordre; i++)
                    {
                        //Charger_images();
                ...
                for(int j=0 ; j < m_ordre ; j++)
                {
                    //tracer l'arrete que si sa "santé" est != de 0 ou strictement>0
