...
void Graphe::affiche_graphe(std::string nomfile)
{
    ...
    while(var_quit_affiche!=1)
    {
        ...
        for(int i=0; i<m_ordre; i++)
        {
            rect(page,m_Sommets[i].getPos_x()-30,m_Sommets[i].getPos_y()-10,m_Sommets[i].getPos_x()+30,m_Sommets[i].getPos_y()+10,makecol(255,255,255));
            textprintf_centre_ex(page,font,m_Sommets[i].getPos_x(),m_Sommets[i].getPos_y(),makecol(255,255,255),-1,m_nom.c_str());
            ...
            if(mouse_x>5 && mouse_x<95 && mouse_y>528 && mouse_y<578)
            {
                if(mouse_b &1)
                {
                    ...
                    ///ERREUR -> voir Sommet.cpp ligne 96
