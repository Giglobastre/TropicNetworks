...
void Graphe::remplissage(int i, stack<int> &Stack)
{
    ...
    //recommence pour toutes les arretes de ce noeud
    for (int i=0 ; i<m_ordre ; i++)
    {
        for (int j=0 ; j<m_ordre ; j++)
        {
...
void Graphe::Kosaraju()
{
    ...
    // marque toutes les sommets qui n'ont pas ete visitée
    for(int i = 0; i < m_ordre; i++)
    {
    ...
    //anayse par l'ordre ou elle sont rentrées dans le stack
...
void Graphe::affiche_graphe(std::string nomfile)
{
    ...
    bouton = create_bitmap(100, SCREEN_H);
    clear_bitmap(page);
    clear_bitmap(bouton);
